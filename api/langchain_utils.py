...
qa_prompt = ChatPromptTemplate.from_messages([
    ("system", """You are an AI assistant whose job is to reply to user queries. You are provided with a block of context information. Your task is to generate an answer to the query using only the details in the context. Do not add any external information or assumptions.
...
3. If the context is relevant and detailed enough, generate a detailed answer covering all the points also citing using only the information provided.
...
def get_rag_chain(query, organization_id: str, workspace_id: str, model: str = "deepseek-r1:1.5b", k: int = 3, file_id: str | None = None):
    ...
    if file_id:
        retriever = FilteredVectorStoreRetrieverWithFilter(
            ...
            metadata_filter={"metadata.file_id": str(file_id)},
